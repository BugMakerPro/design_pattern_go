...
> [实践](https://blog.csdn.net/hfreeman2008/article/details/52344022?utm_medium=distribute.pc_relevant.none-task-blog-BlogCommendFromMachineLearnPai2-2.control&dist_request_id=6019aabf-1599-4a05-90c9-a93018a77f3e&depth_1-utm_source=distribute.pc_relevant.none-task-blog-BlogCommendFromMachineLearnPai2-2.control "Markdown")  

那么是不是就一定不能修改原来的类的，当然不是了，我们都是成年人了，要清楚的认识到，这个世界不是非黑即白的。当我们发现原来的类已经烂到家了，当然在有条件的情况下及时重构，避免系统加速腐败。

#### 3 里氏替换原则（Liskov Substitution Principle）
这个原则的的提出则可以是一位女性Barbara Liskov，现在应该还健在吧，其实计算机这个行当的从业人员比较幸福，我们的祖师爷基本都健在，不像一些其他行业，都死了不知道多少年了，显得很神秘。  
定义如下:  
...
例如我们有如下的代码，一个图形的基类Shap,以及它的两个子类Rectangle ，Triangle,按照里式替换原则，所有使用Shape的地方都可以安全的替换成其子类。
...
结束画图
```
如上代码所示：本来`drawShape()`函数需要一个`Shape`的实例，而我们却传给他一个其子类的实例，但是它正常工作了。我们使用Shape的子类Triangle的实例来替换Shape的实例，程序工作正常。这个原则也非常重要而常用，面向抽象编程。
...
![avatar](https://github.com/BugMakerPro/design_pattern_go/blob/main/99_pic/v2-60e199f1f93e3932752f07a7456e46d8_1440w.jpg)  
